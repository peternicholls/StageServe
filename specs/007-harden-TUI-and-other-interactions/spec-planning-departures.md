...
- `not_project` decision bar changed from setup-help-first to `Set up this directory as a project` (`init_here` → `stage init`, navigates to `project_missing_config`) with `Pick a different folder` as the other plain-language choice. Rationale: a user who has run `stage` from a real project directory is more likely to want to convert that directory than to be redirected to machine setup.
- `unknown_error` recovery panel must surface a concrete ordered next-step sequence (`stage doctor`, `stage status`, `stage logs`) rather than placeholder copy. The highlighted default runs the next least-invasive recovery step; direct commands remain visible through the footer. Rationale: the prototype showed that a recovery screen with no concrete next step dead-ends the user.
