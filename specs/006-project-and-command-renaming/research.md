...
These are active user-facing surfaces where `stacklane` is currently canonical and will need explicit cutover work.
...
- Stacklane is the current primary product name in headings and opening copy.
- `stacklane` is the current canonical command in install, setup, runtime, and migration examples.
- Installer and manual download examples currently fetch `stacklane_*` assets and place `stacklane` on `PATH`.
...
- root command uses `Use: "stacklane"`
- short and long descriptions are Stacklane-branded
- some generated or inline next-step guidance still tells users to run `stacklane up`
...
- active docs describe `stacklane` binary download and install flow
- active docs refer to `stacklane-bin` and `stacklane` as build/install outputs
...
- spec 005 quickstart and task artifacts still teach `stacklane setup`, `stacklane doctor`, and `stacklane init`
- spec 004 research and contract artifacts still describe `stacklane` as the active command surface
...
- `.stacklane-state`
- `STACKLANE_*`
- `stln-*` → `stage-*` (Docker Compose project names, network names, volume names in `core/config/loader.go` and compose YAML files)
- internal file-system and runtime names that derive from the old boundary above
- package/import paths such as `github.com/peternicholls/stacklane/...` if they remain repository-owned and active by closeout

Evidence observed in active code:

- command code and tests refer to `.env.stacklane` and `.stacklane-state`
...
These surfaces may still mention Stacklane as part of historical analysis or migration framing, but they must not remain the canonical operator story after cutover.
...
- the rename plan therefore has no in-repo ambiguity, but it does increase machine-level collision risk because the new command is much more generic than `stacklane`
...
3. Internal `stacklane` surfaces are deeply present in both docs and tests, so the plan needs an explicit later-phase rename sweep rather than pretending the external cutover alone finishes the job.
4. The generic command name `stage` raises a machine-level collision risk that the previous `stacklane` command did not.
