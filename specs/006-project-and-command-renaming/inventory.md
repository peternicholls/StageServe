# Rename Inventory: Active `stage` Surfaces
...
| Compatibility shim | temporary `stage` forwarding path if retained | rename now, then remove | It may exist only as a transition aid, not as a closeout surface | shim file/path to be decided in Gate A |
| Go module path | `go.mod` `module github.com/peternicholls/stage` | rename later in this spec | This is active code identity, but riskier than the command cutover and should follow the public command rename | `go.mod` |
| Go import paths | imports under `cmd/`, `core/`, `infra/`, `observability/`, `platform/` still point at `github.com/peternicholls/stage/...` | rename later in this spec | Must be migrated together with the module path to avoid broken builds | `cmd/stacklane/main.go` |
| Package and directory names | `cmd/stacklane/**` and any repository-owned paths still carrying `stage` | rename later in this spec | Path/layout rename is active code identity work and should be sliced carefully | `cmd/stage` |
| Internal env and state filenames | `.env.stageserve`, `.stageserve-state`, `.stage-local` references, generated text that still emits them | rename later in this spec | They are active surfaces today but must not remain by closeout | `core/config/loader.go` |
...
| Runtime Docker prefix | `stage-<slug>` compose project, `stage-<slug>-runtime` network, `stage-<slug>-db-data` volume, `stage-shared` gateway project/network — set in `core/config/loader.go` lines ~297-334 | rename later in this spec | Must rename to `stage-*` by closeout; no live migration needed (local dev only, `docker system prune` acceptable) | `core/config/loader.go` |
...
1. What are the final replacement names for `.env.stageserve`, `.stageserve-state`, and `STAGESERVE_*`?
...
3. Will a temporary `stage` shim be shipped at all, and if yes, what is the exact removal point before closeout?
...
5. ~~Does `stage-*` stay as the runtime prefix?~~ **Decided**: rename `stage-*` → `stage-*`. No live migration required; local dev only.
